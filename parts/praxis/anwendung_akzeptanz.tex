...
Dies lösen wir, indem wir auf der View\index{Rails!View} der Startseite eines einbauen:
...
Jetzt folgt die Implementierung der Suche im Jobs-Controller\index{Rails!Controller}:
...
Im Jobs-Controller\index{Rails!Controller} implementieren wir die Bereitstellung der Jobs mittels einer Datenbankabfrage. Dabei nutzen wir die Datenbankfunktion von ActiveRecord\index{ActiveRecord} und führen eine partielle Match-Suche der Suchparameter über die Tabellenspalte "`title"' aus. ActiveRecord übernimmt das sichere Escapen des übergeben Strings selbstständig in den dafür vorgesehenen Platzhalter "`?"'. Das Ergebnis speichern wir in der Instanzvariable @jobs, und stellen es so der View zur Verfügung.


Damit nun der Test \index{Test}besteht, muss als letztes noch eine View\index{Rails!View} implementiert werden, die (zumindest) die Titel aller in der Suche gefundenen Jobs ausgibt, hier z.B. als Überschrift.
% SNIPPET: 
% SNIPPET: 
%                                                                                                                                          
% <% @jobs.each do |job| %>                                                                                                                
...
Ein guter Ansatz wäre z.B. die Suchlogik aus dem Controller\index{Rails!Controller} in die Job-Modelklasse auszulagern.
...
Nun können wir die neu implementierte Funktion in unserem Controller\index{Rails!Controller} verwenden.
