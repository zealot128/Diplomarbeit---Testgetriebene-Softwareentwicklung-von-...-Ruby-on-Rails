...
\begin{itemize}
 ...
 \item die dazugehörige \textbf{Testklasse (\verb|app/unit/job_test.rb|) von Test/Unit (vgl. Abschnitt \ref{sec:rubyTestUnit})
...
Danach können wir die Rails\index{Ruby-on-Rails}-Test-Suite\index{Test!Test-Suite} mithile eines weitere Rake\index{Ruby!Rake}-Tasks auch schon ausführen. Dieser Rake-Task legt selbständig eine Testdaten\index{Test!Testdaten}bank an, führt offene Migrationen darauf aus und führt dann alle vorhanden Tests aus.
...
Da wir noch keine Validierung\index{Ruby-on-Rails!Validierung}skritieren implementiert haben, gibt \texttt{save} \texttt{true} zurück und die Zusicherung fehl.
...
\texttt{validates} ist eine Funktion aus der ActiveRecord\index{ActiveRecord} Bibliothek, die zwei Parameter entgegennimmt: Der erste ist die Spalte, auf der sich die Validierung\index{Ruby-on-Rails!Validierung} bezieht, als Zweites folgt eine Liste an Validierungskritieren. Hier ist das Kriterium \texttt{presence}, also das Vorhandensein eines nicht-leeren Wertes für diese Spalte. Weitere Kriterien sind z.B. Format, Länge, Minimum, Maximum oder auch selbst definierte Kriterien.
