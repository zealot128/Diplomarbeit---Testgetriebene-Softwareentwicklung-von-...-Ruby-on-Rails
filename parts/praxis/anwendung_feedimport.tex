...
\end{lstlisting}
Das Mock\index{Test-Double!Mock}objekt hat unseren Test \index{Test}fehlschlagen lassen, ohne dass wir selbst eine Assertion festgelegt hätten. Da wir bisher noch keine Implementation eines Netzwerkzugriffes durch HTTParty implementiert haben, schlägt der Test fehl.
...
\tddgreen
Die Funktion initialize stellt innerhalb von Ruby\index{Ruby} den Konstruktor dar. Dort rufen wir unseren Netzwerkzugriff auf, der allerdings durch unser definiertes Mock\index{Test-Double!Mock}-Objekt abgefangen wird. Dies stellt die definierte Erwartung zufrieden und der Test \index{Test}besteht.
