...
Features der \textbf{Kategorie A} sollen in Zukunft Akzeptanztest\index{Test!Akzeptanztest}getrieben entwickelt werden. Die Entwicklung verläuft nach dem Schema, dass in Abschnitt \ref{sec:attd} vorgestellt wurde. Die Akzeptanztests sollen in Cucumber geschrieben werden, und mittels Capybara\index{Capybara} auf simulierten Browsern ausgeführt werden (vgl. Abschnitt \ref{sec:cucumber}).
...
\paragraph{Werkzeuge für Code-Metriken}  Für die Generierung von \glossarpl{metrik} dient das \glossar{gem} \textbf{metric-fu}, welches seinerseits über verschiedene Code-Metriken Zusammenfassungen bildet und diese auch in der zeitlichen Entwicklung darstellt. Darunter fallen z.B. die Zyklomatische Komplexität, den Grad der Duplikationen, verschiedene Code-Smells, Nutzung der Versionsverwaltung und \glossar{Testabdeckung}\index{Test!Testabdeckung}.
