...
\begin{enumerate}[A.]
 ...
 \item Features, welche nur dem Admin sichtbar sind oder welche im Backend ausgeführt werden $\to$ Reporting, Statistiken, Indizierung der Datenbank\index{Datenbank}, Cron-Scripte, Caching, ...
\end{enumerate}

Features der \textbf{Kategorie A} sollen in Zukunft Akzeptanztest\index{Akzeptanztest}getrieben entwickelt werden. Die Entwicklung verläuft nach dem Schema, dass in Abschnitt \ref{sec:attd} vorgestellt wurde. Die Akzeptanztests sollen in Cucumber\index{Cucumber} geschrieben werden und mittels Capybara\index{Capybara} auf simulierten Browser\index{Browser!Simulierter}n ausgeführt werden (vgl. Abschnitt \ref{sec:cucumber}).
