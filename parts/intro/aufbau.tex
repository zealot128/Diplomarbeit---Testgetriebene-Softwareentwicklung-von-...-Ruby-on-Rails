...
Das aktuelle einleitende Kapitel beschreibt die Motivation und Hintergründe, warum die pludoni GmbH die Testgetrieben Entwicklung mit Ruby on Rails\index{Ruby-on-Rails} verwenden möchte und gibt eine kurze Projektbeschreibung über das Projekt IT-Jobs\index{IT-Jobs-Projekt}, welches parallel zu dieser Arbeit enstand.

Da die Testgetriebene\index{TDD} Softwareentwicklung auf dem dynamischen Unittest\index{Test!Unittest} basiert, werden ist das Kapitel \fullref{sec:test} dem Testen im Allgemeinen gewidmet. Danach erfolgt eine Einführung in die \textit{\nameref{sec:tdd}} im Kapitel \ref{sec:tdd}. \\
...
Das Kapitel \fullref{sec:metriken} zeigt auf, welche Möglichkeiten existieren, um den Erfolg der Testgetriebene\index{TDD}n Entwicklung praktisch anhand von Kennziffern nachzuweisen und den Prozess zu kontrollieren.
...
Danach schauen wir uns konkrete Ergebnisse im Kapitel \fullref{sec:auswertung} aus den Code-Metrik\index{Code-Metrik}en an und vergleichen diese mit anderen Projekten der pludoni GmbH und als Ausblick auch mit einigen bekannteren Rails\index{Ruby-on-Rails}-Projekten.
...
Im Appendix gibt es Hinweise für die Erstellung guter Tests aus der Literatur und die Anleitung, wie eine effektive Browsersimulation\index{Browser!Simulierter}ssumgebung eingerichtet wird. Am Ende der Arbeit erfolgt eine Auflistung der verwendeten Abbildungen, Quellcodes und das Literaturverzeichnis.
